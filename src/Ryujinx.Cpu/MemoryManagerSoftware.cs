...
namespace Ryujinx.Cpu
{
    ...
    /// </summary>
    public class MemoryManagerSoftware : MemoryManagerBase, IWritableBlock
    {
        ...
        public MemoryTracking Tracking { get; protected set; }
        ...
        public MemoryManagerSoftware(MemoryBlock backingMemory, ulong addressSpaceSize, InvalidAccessHandler invalidAccessHandler = null)
        {
        ...
        protected void PtMap(ulong va, ulong pa, ulong size)
        {
            while (size != 0)
            {
        ...
        protected void PtUnmap(ulong va, ulong size)
        {
            while (size != 0)
            {
                _pageTable.Unmap(va);
        ...
        public virtual void TrackingReprotect(ulong va, ulong size, MemoryPermission protection)
        {
        ...
        protected void AddMapping(ulong va, ulong size)
        {
            int pages = GetPagesCount(va, size, out _);
            ulong pageStart = va >> PageBits;
            ulong pageEnd = pageStart + (ulong)pages;
        ...
        protected void RemoveMapping(ulong va, ulong size)
        {
            int pages = GetPagesCount(va, size, out _);
            ulong pageStart = va >> PageBits;
            ulong pageEnd = pageStart + (ulong)pages;
